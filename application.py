...
class Application:
    ...
    def toggle_tts(self):
        if not self.tts_playing:
            ...
            tk_async_execute.async_execute(
                self.text_to_speech.play(self.text), visible=False)
        else:
    ...
    def build_window(self):
        ...
        sidebar_frame = tkinter.Frame(
            self.root, width=200, height=400, bg="skyblue")
        sidebar_frame.pack(padx=5, pady=5, side=tkinter.LEFT, fill=tkinter.Y)
        tkinter.Label(sidebar_frame, text="Options",
                      bg="skyblue").pack(padx=5, pady=5)

        # voice selection menu
        voice_selection_frame_color = "#777f8c"
        voice_selection_frame = tkinter.Frame(
            sidebar_frame, width=100, height=50, bg=voice_selection_frame_color)
        voice_selection_frame.pack(padx=5, pady=5, fill=None, expand=False)
        tkinter.Label(voice_selection_frame, text="Voice", fg="white",
                      bg=voice_selection_frame_color).pack(padx=5, pady=5)
        voice_options = self.text_to_speech.get_voice_options()
        voice_names = []
        voice_dict = {str.split(voice.name, "-")
                      [0]: voice.id for voice in voice_options}
        ...
        tkinter.OptionMenu(
            voice_selection_frame, voice_selection_var, *voice_names).pack(padx=5, pady=2)
        voice_test_button = tkinter.Button(voice_selection_frame, text="Test", command=lambda: tk_async_execute.async_execute(
            self.text_to_speech.test_voice(), visible=False), width=10)
        voice_test_button.pack(padx=5, pady=2, side=tkinter.LEFT)

        # file selection menu
        file_selection_frame_color = "#777f8c"
        file_selection_frame = tkinter.Frame(
            sidebar_frame, width=100, height=50, bg=file_selection_frame_color)
        file_selection_frame.pack(padx=5, pady=5, fill=tkinter.X)
        tkinter.Label(file_selection_frame, text="PDF", fg="white",
                      bg=file_selection_frame_color).pack(padx=5, pady=5)
        tkinter.Button(file_selection_frame, text="Select PDF",
                       command=self.file_picker, width=20).pack(padx=5, pady=2)

        # primary frame
        primary_frame = tkinter.Frame(
            self.root, width=400, height=700, bg="grey")
        ...
        def on_text_modified(event):
            # "end-1c" removes the trailing newline.
        ...
        self.start_stop_button = tkinter.Button(
            text_tab, text="Start", command=self.toggle_tts, width=10)
        self.start_stop_button.pack(padx=5, pady=10, side=tkinter.LEFT)
        export_text_button = tkinter.Button(
            text_tab, text="Export", command=self.export, width=10)
